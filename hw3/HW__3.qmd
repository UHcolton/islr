...
header-includes:
  - \usepackage{soul}
  - \input{c:/texlive/2024/texmf-dist/tex/generic/soul/soul.sty}
---
...
1.  $$ \text{SE}_B(\hat{Y})=\sqrt{\frac{1}{B-1}\sum_{r=1}^{B}\left(\hat{Y}^{*r}-\frac{1}{B}\sum_{r'=1}^{B}\hat{Y}^{*r'}\right)^2} $$
...
After splitting 4 times, the validation set test error rate varies by $0.0215\%$.

$\left(\mu,\ \sigma\right)=(2.57, \ 0.1465)$ The inconsistency here is due to the observations being randomly put into either the training or validation set during the splits.
...
Comment: With the bootstrap, we get an estimated $10\text{th}$ percentile value of $12.75$, a standard error of $0.5113$. This small standard error, compared to the percentile value of $12.75$, indicates that the estimate is fairly precise.
